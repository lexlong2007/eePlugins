...
class MSNWeatherThingSpeak(Converter, object):
    def __init__(self, type):
        ...
        try:
            ...
            if DBG: printDEBUG('MSNWeatherThingSpeak:__init__',"Exception: '%s'" % str(e))

    def currEntryChanged(self, configElement = None): 
        if DBG: printDEBUG('MSNWeatherThingSpeak:configChanged',">>>")
        self.ThingSpeakItems = {}
        try:
            currEntry = config.plugins.WeatherPlugin.currEntry.value
            ...
            if DBG: printDEBUG('MSNWeatherThingSpeak:configChanged',"Exception: '%s'" % str(e))
    ...
    def getText(self): #self.mode = ('name','description','field1Name','field2Name','ObservationTime','field1Value','field1Status','field2Value','field2Status')
        if DBG: printDEBUG('MSNWeatherThingSpeak:getText','>>> self.mode="%s", currEntry=%s' % (self.mode, config.plugins.WeatherPlugin.currEntry.value))
        self.syncItems()
        retTXT = ''
        if len(self.ThingSpeakItems) > 0:
            try:
                ...
                if DBG: printDEBUG('MSNWeatherThingSpeak:getText','Exception="%s"' % str(e))
