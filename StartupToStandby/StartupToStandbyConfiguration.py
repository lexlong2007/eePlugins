...
class StartupToStandbyConfiguration(Screen, ConfigListScreen):
	...
	def __init__(self, session):
		...
		ConfigListScreen.__init__(
			self,
			[
				getConfigListEntry(_("On every startup:"), config.plugins.startuptostandby.enabled),
				getConfigListEntry(_("Only when tuner was in standby:"), config.plugins.startuptostandby.WhenWasInStandbyOnly)
			],
			session = session,
