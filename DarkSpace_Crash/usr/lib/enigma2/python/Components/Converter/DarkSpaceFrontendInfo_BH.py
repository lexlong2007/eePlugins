...
class DarkSpaceFrontendInfo_BH(Converter, object):
	BER = 0
	SNR = 1
	AGC = 2
	LOCK = 3
	...
	@cached
	def getText(self):
		assert self.type not in (self.LOCK, self.SLOT_NUMBER), "the text output of FrontendInfo_BH cannot be used for lock info"
		percent = None
	...
	@cached
	def getBool(self):
		assert self.type in (self.LOCK, self.BER), "the boolean output of FrontendInfo_BH can only be used for lock or BER info"
	...
	@cached
	def getValue(self):
		assert self.type != self.LOCK, "the value/range output of FrontendInfo_BH can not be used for lock info"
