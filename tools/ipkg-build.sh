...
#tar -C $pkg_dir -czf $tmp_dir/data.tar.gz . --exclude=$pkg_dir/CONTROL
#
# gutemine's fix for --exclude bug
#
FILESWITHOUTCONTROL=`cd $pkg_dir; ls -1 | grep -v CONTROL` 
#echo $FILESWITHOUTCONTROL

tar -C $pkg_dir -czf $tmp_dir/data.tar.gz $FILESWITHOUTCONTROL
...
if [ -f /usr/bin/enigma2 ]; then
   if [ ! -f /usr/local/bin/ar ]; then
      BUSYBOX=`ls -F /usr/bin/ar | grep @ | wc -l`
      if [ $BUSYBOX -eq 1 ]; then
         echo ar seems to be from busybox - not capable to create archive 
         echo Install ar from binutils at /usr/local/bin
         echo or replace link from /usr/bin/ar to busybox with real ar binary
         exit 0
      fi
   fi
fi
...
echo "Packaged content was moved at $pkg_dir into $pkg_file"
