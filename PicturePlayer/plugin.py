...
from Components.config import config, NoSave, ConfigNothing, ConfigSubsection, ConfigInteger, ConfigSelection, ConfigText, ConfigEnableDisable, KEY_LEFT, KEY_RIGHT, KEY_0, getConfigListEntry
...
class picshow(Screen):
    ...
    def showThumb(self):
        if not self.filelist.canDescent():
            if self.filelist.getCurrentDirectory() and self.filelist.getFilename():
                if config.pic.autoRotate.value and autorotate(self.filelist.getCurrentDirectory() + self.filelist.getFilename()):
    ...
    def KeyExit(self):
        ...
        if self.LastPlayedService: self.session.nav.playService(self.LastPlayedService)
...
class Pic_Setup(Screen, ConfigListScreen):
    ...
    def createSetup(self):
        ...
        if config.pic.autoRotate.value:
            self.list.append(getConfigListEntry(_("PIL loaded, rotating pic enabled :)"), config.pic.separator))
        else:
            self.list.append(getConfigListEntry(_("PIL not loaded, rotating pic disabled :("), config.pic.separator))
    ...
    def showPic(self, picInfo=""):
        for x in range(len(self.Thumbnaillist)):
            if self.Thumbnaillist[x][0] == 0:
                if config.pic.autoRotate.value and autorotate(self.Thumbnaillist[x][2]):
                    if self.picload.getThumbnail('/tmp/rotated.jpg') == 1:
                        self.ThumbTimer.start(500, True)
...
class Pic_Full_View(Screen):
    ...
    def start_decode(self):
        if config.pic.autoRotate.value and autorotate(self.filelist[self.index]):
...
def Plugins(**kwargs):
    return [PluginDescriptor(name=_("PicturePlayer"), icon="pictureplayer.png", where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, fnc=main),
