...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            r = [(client.parseDOM(i, 'a', ret='href'), client.parseDOM(i, 'span', attrs={'class': 'tt'}),
                  client.parseDOM(i, 'span', attrs={'class': 'year'})) for i in r]
    ...
    def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            ...
            if not r:
                ...
                r = client.parseDOM(r, 'div', attrs={'class': 'item'})
                r = zip(client.parseDOM(r, 'a', ret='href'), client.parseDOM(r, 'span', attrs={'class': 'tt'}),
                        client.parseDOM(r, 'span', attrs={'class': 'year'}))
                r = [(i[0], re.sub('(?:\(|\s)\(' + year + '.+', '', i[1]).strip().encode('utf-8'), i[2]) for i in r if
                     len(i[0]) > 0 and '/series/' in i[0] and len(i[1]) > 0 and len(i[2]) > 0]
            ...
            for u in f:
                try:
                    ...
                    if 'stream/ol.php' in u:
                        ...
                        sources.append(
                            {'source': 'openload.co', 'quality': 'HD', 'language': 'en', 'url': url, 'direct': False,
                             'debridonly': False})
                    ...
                    if 'stream/play.php' in u:
                        ...
                        for i in links:
                            try:
                                ...
                                sources.append(
                                    {'source': 'gvideo', 'quality': quality, 'language': 'en', 'url': i['file'],
