...
class source:
    def __init__(self):
        self.priority = 1
        self.language = ['en']
        self.domains = ['pubfilms.tv']
        self.base_link = 'https://pubfilms.tv'
        ...
        try:
            ...
            if season <> '0':
                try:
                    ...
                    page = \
                        [re.findall(r'class=\'title_new\'>.*?Season\s+%s.*?<.*?href="([^"]+)">' % season, r, re.DOTALL)
                         for
                         r in result if
                         bool(re.search(r'class=\'title_new\'>.*?Season\s+%s.*?<' % season, r, re.DOTALL))][
                            0][0]
                ...
                try:
                    ...
                    result = client.parseDOM(result, 'div', attrs={'class': 'recent-item'})
                    page = \
                        [re.findall(r'class=\'title_new\'>.*?\(%s\).*?href="([^"]+)"' % year, r, re.DOTALL) for r in
                         result
