...
class source:
    ...
    def resolve(self, url):
        try:
            ...
            import requests
            p1 = requests.post('http://freeputlockers.org/ip.file/swf/plugins/ipplugins.php', data=post,
                               headers={'referer': urldata['url'], 'X-Requested-With': 'XMLHttpRequest'}).content
            p1 = json.loads(p1)
            p2 = requests.get('http://freeputlockers.org/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0' % (
