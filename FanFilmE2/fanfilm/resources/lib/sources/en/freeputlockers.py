...
class source:
    ...
    def resolve(self, url):
        try:
            ...
            p1 = client.request('http://freeputlockers.org/ip.file/swf/plugins/ipplugins.php', post=post,
                                referer=urldata['url'], XHR=True)
            p1 = json.loads(p1)
            p2 = client.request('http://freeputlockers.org/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0' % (
            ...
            n = p3['status']
            if n == False:
                p2 = client.request('http://freeputlockers.org/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=1' % (
                    p1['s'], urldata['data-server']))
                p2 = json.loads(p2)
