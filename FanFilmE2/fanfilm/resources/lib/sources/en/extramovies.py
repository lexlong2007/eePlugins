...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        if 1:  # try:
            ...
            match = re.compile(
                'class="thumbnail">.+?href="(.+?)" title="(.+?)".+?class="rdate">(.+?)</span>.+?</article>',
                re.DOTALL).findall(html)  # Regex info on results page
        ...
        if 1:  # try:
            ...
            for links in stream:
                if 'video.php' in links:
                    link = 'https://lh3.googleusercontent.com/' + links.split('=')[1].replace('&#038;s',
                                                                                              '') + '=m18|User-Agent=Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:61.0) Gecko/20100101 Firefox/61.0'
                    count += 1
                    self.sources2.append(
                        {'source': 'Google', 'quality': res, 'language': 'en', 'url': link, 'direct': True,
                         'debridonly': False})
                    ...
                    if 'Www' not in host:
                        count += 1
                        self.sources2.append(
                            {'source': host, 'quality': res, 'language': 'en', 'url': link, 'direct': False,
                             'debridonly': False})
            for link in Regex:
                try:
