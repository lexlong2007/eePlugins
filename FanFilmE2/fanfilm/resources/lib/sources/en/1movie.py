...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            url = self.__search([data['tvshowtitle']] + source_utils.aliases_to_array(eval(data['aliases'])),
                                data['year'], season)
            if not url: return
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            r = client.request(urlparse.urljoin(self.base_link, self.player_link), post={'id': p[0]}, referer=ref,
                               XHR=True)
            ...
            if '1movies.' in link:
                ...
                r = [(match[1], match[0]) for match in
                     re.findall('''['"]?file['"]?\s*:\s*['"]([^'"]+)['"][^}]*['"]?label['"]?\s*:\s*['"]([^'"]*)''', r,
                                re.DOTALL)]
                ...
                if 'google' in link and not urls and directstream.googletag(link):
                    host = 'gvideo';
                    direct = True;
                    urls = [
                        {'quality': directstream.googletag(link)[0]['quality'], 'url': link}]
                elif 'ok.ru' in link:
                    host = 'vk';
                    direct = True;
