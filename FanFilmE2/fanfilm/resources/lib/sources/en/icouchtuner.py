    ...
    def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            ...
            for content in results:
                show_url, url_text = \
                    re.compile('href="(.+?)" rel="bookmark" title="(.+?)"', re.DOTALL).findall(content)[0]
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            for content in posttabs:
                ...
                sources.append({'source': host, 'quality': 'SD', 'language': 'en', 'url': vid_url, 'direct': False,
                                'debridonly': False})
            return sources
        except:
            failure = traceback.format_exc()
