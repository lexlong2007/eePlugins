...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            result = re.compile('alt="(.+?)".+?<h2><a href="(.+?)".+?</h2>.+?>(.+?)</p>', re.DOTALL).findall(
                str(results))
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            for link, res in match:
                ...
                sources.append({'source': 'DirectLink', 'quality': res, 'language': 'en', 'url': vid_url, 'info': '',
            ...
            for link in match2:
                ...
                if 'flashx' not in link:
                    sources.append(
                        {'source': host, 'quality': res, 'language': 'en', 'url': link, 'info': 'AC3', 'direct': False,
                         'debridonly': False})
