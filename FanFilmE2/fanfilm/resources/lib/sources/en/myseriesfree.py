...
class source:
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            try:
                ...
                url = [i.attrs['href'] for i in items if
                       bool(re.compile('"datePublished">%s' % premiered).search(i.content))][0]
            except:
                url = None
                pass
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            links = re.compile('<i class="fa fa-youtube link-logo"></i>([^<]+).*?href="([^"]+)"\s+class="watch',
                               re.DOTALL).findall(result)
            ...
            for pair in links:
                ...
                sources.append({'source': host, 'quality': quality, 'language': 'en', 'url': link, 'direct': direct,
                                'debridonly': False})
