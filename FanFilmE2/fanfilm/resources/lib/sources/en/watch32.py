...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            varid = re.compile('var frame_url = "(.+?)"', re.DOTALL).findall(search_results)[0].replace('/embed/',
                                                                                                        '/streamdrive/info/')
            ...
            for link in links:
                ...
                if '1080' in res_chk:
                    ...
                    quality = 'DVD'
                sources.append(
                    {'source': 'Googlelink', 'quality': quality, 'language': 'en', 'url': vid_url, 'direct': False,
                     'debridonly': False})
            return sources
        except:
            failure = traceback.format_exc()
