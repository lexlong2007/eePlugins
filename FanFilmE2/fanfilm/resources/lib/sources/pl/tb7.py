...
from resources.lib.libraries import client, control, cache
...
class source:
    def __init__(self):
        ...
        self.search_link = 'https://tb7.pl/'  # post
    ...
    def search(self, title, localtitle, year=''):
        try:
            ...
            for title in titles:
                ...
                self.session.post('https://tb7.pl/mojekonto/szukaj', data=data).content
                ...
                self.session.post('https://tb7.pl/mojekonto/szukaj/1', headers=headers,
                                  data=data)
                r = self.session.post('https://tb7.pl/mojekonto/szukaj/1', headers=headers,
                                      data=data).content
    ...
    def sources(self, rows, hostDict, hostprDict):
        ...
        try:
            if type(rows) == tuple:
                ...
                sources.append(
                    {'source': 'tb7', 'quality': quality, 'language': info[0], 'url': rows[0],
                     'info': 'Moje Pliki',
                     'direct': True,
                     'debridonly': False})
                return sources
            for row in rows:
                try:
                    ...
                    sources.append(
                        {'source': 'tb7', 'quality': quality, 'language': info[0], 'url': link,
                         'info': size + ' ' + info2,
                         'direct': True,
                         'debridonly': False})
                except:
    ...
    def resolve(self, url):
        ...
        result = self.session.get(mojekonto, headers=headers).content
        ...
        if autotb7 == 'false':
            limit = self.session.get(self.base_link, headers=headers).content
            limit = client.parseDOM(limit, 'div', attrs={'class': "textPremium"})
            limit = str(client.parseDOM(limit, 'b')[-1])

            import xbmcgui
            ret = xbmcgui.Dialog().yesno('TB7', 'Chcesz wykorzystać transfer ze swojego konta tb7 aby odtworzyć tę pozycję?\n Aktualnie posiadasz: [B]%s[/B] transferu' % limit)
            if not ret:
                return
        ...
        self.session.post('https://tb7.pl/mojekonto/sciagaj', data=data, headers=headers)
        ...
        content = self.session.post('https://tb7.pl/mojekonto/sciagaj', data=data, headers=headers).content
