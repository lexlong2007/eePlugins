...
class source:
    ...
    def check_titles(self, cleaned_titles, found_titles):
        test = cleaned_titles[0] == cleantitle.get(found_titles[0]) or cleaned_titles[1] == cleantitle.get(
            found_titles[1]) or cleaned_titles[0] == cleantitle.get(found_titles[1]) or cleaned_titles[
                   1] == cleantitle.get(found_titles[0])
        return test
    ...
    def extract_sources(self, transl_type, links, typ, html):
        sources = []
        if typ == "episode":
            test = re.search("""data-code="(.*?)" data-code2="(.*?)">""", html)
            code = test.group(1)
            code2 = test.group(2)
        if typ == "movie":
            test = re.search("""data-code="(.*?)">""", html)
            code = test.group(1)
            code2 = "undefinded"
        title = client.parseDOM(html, 'h2', attrs={'class': 'title'})[0]
        try:
            title2 = client.parseDOM(html, 'h3', attrs={'class': 'title_org'})[0]
        except:
            title2 = ''
        ...
        for i in range(0, len(result)):
            ...
            sources.append(
                {'source': host, 'quality': q, 'language': lang, 'url': (typ, code, code2, data_refs[i], title, title2),
    ...
    def resolve(self, url):
        try:
            url_to_exec = str(urlparse.urljoin(self.base_link, self.url_transl) % (
            url[0], url[1], url[2], url[3], url[4], url[5])).replace(" ", "%20")
            headers = {
                'User-Agent': "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3555.0 Safari/537.36"}
            result = client.request(url_to_exec, headers=headers)
