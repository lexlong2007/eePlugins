    ...
    for i in result:
        try:
            ...
            cm.append(('Remove from Queue',
                       'RunPlugin(%s?action=removeDownload&url=%s)' % (sys.argv[0], urllib.quote_plus(i['url']))))
...
def addDownload(name, url, image, provider=None):
    ...
    try:
        ...
        no = control.yesnoDialog(dest, 'Complete file is' + size, 'Continue with download?',
                                 name + ' - ' + 'Confirm Download', 'Confirm', 'Cancel')
...
def startDownload():
    if downloadPath == '':
        return control.infoDialog('You need to set your download folder in addon settings first',
...
class MyDownloads(pyxbmct.AddonDialogWindow):
    ...
    def set_info_controls(self):
        self.placeControl(
            pyxbmct.Label('[B][COLOR gold]' + control.window.getProperty(property + '.name') + '[/COLOR][/B]',
                          alignment=pyxbmct.ALIGN_CENTER), 0, 1, 1, 1)

        self.placeControl(pyxbmct.Label('Total File Size', alignment=pyxbmct.ALIGN_CENTER), 1, 0)
        self.placeControl(pyxbmct.Label(control.window.getProperty(property + '.size'), alignment=pyxbmct.ALIGN_CENTER),
                          2, 0)

        self.placeControl(pyxbmct.Label('Downloaded', alignment=pyxbmct.ALIGN_CENTER), 1, 1)
        self.placeControl(
            pyxbmct.Label(control.window.getProperty(property + '.percent'), alignment=pyxbmct.ALIGN_CENTER), 2, 1)

        self.placeControl(pyxbmct.Label('Download Speed', alignment=pyxbmct.ALIGN_CENTER), 1, 2)
        self.placeControl(
            pyxbmct.Label(control.window.getProperty(property + '.speed'), alignment=pyxbmct.ALIGN_CENTER), 2, 2)
            ...
            while True:
                ...
                if (resumable and errors > 0) or errors >= 10:
                    ...
                    if resumable:
                        ...
                        try:
                            resp = urllib2.urlopen(urllib2.Request(url, headers=h), timeout=10)
                        except:
                            resp = None
                    else:
