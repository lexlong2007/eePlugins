...
class Cloudflare:
                ...
                if len(line) and '=' in line:
                    ...
                    if '/' in expression and not 'function' in expression:
                        ...
                        if '/' in expression:
                            ...
                            subsecs[funcSubsecsIndex], extraValue = self.sampleDomainFunction(subsecs[funcSubsecsIndex],
                                                                                              self.domain)
                            line_val = self.parseJSString(subsecs[0]) / float(
                                self.parseJSString(subsecs[1]) + extraValue)
                        else:
    ...
    def parseJSString(self, s):
        val = int(
            eval(
                s.replace('!+[]', '1').replace('!![]', '1').replace('[]', '0').replace('(', 'str(').replace('(+str',
                                                                                                            '(str').strip(
                    '+')
            )
        )
