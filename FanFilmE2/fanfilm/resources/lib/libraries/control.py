...
def appearance():
    appearance = setting('appearance.1').lower() if condVisibility(
        'System.HasAddon(script.fanfilm.media)') else setting('appearance.alt').lower()
    return appearance
...
def metaFile():
    if condVisibility('System.HasAddon(script.fanfilm.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.fanfilm.metadata').getAddonInfo('path'), 'resources', 'data',
                            'meta.db')
