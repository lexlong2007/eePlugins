...
from resources.lib.libraries import playcount, sources
...
class player(xbmc.Player):
    ...
    def run(self, title, year, season, episode, imdb, tvdb, url, meta):
        try:
            ...
            self.name = urllib.quote_plus(title) + urllib.quote_plus(
                ' (%s)' % year) if self.content == 'movie' else urllib.quote_plus(title) + urllib.quote_plus(
            ...
            item.setArt(
                {'icon': thumb, 'thumb': thumb, 'poster': poster, 'tvshow.poster': poster, 'season.poster': poster})
    ...
    def getMeta(self, meta):
        ...
        try:
            ...
            meta = [i for i in meta if self.year == str(i['year']) and (
                        t == cleantitle.get(i['title']) or t == cleantitle.get(i['originaltitle']))][0]
    ...
    def keepPlaybackAlive(self, meta):
        ...
        if self.content == 'movie':
            ...
            overlay = playcount.getEpisodeOverlay(playcount.getTVShowIndicators(), self.imdb, self.tvdb, self.season,
                                                  self.episode)
    ...
    def onPlayBackStopped(self):
        ...
        if self.currentTime == 0 or self.totalTime == 0 or self.currentTime <= 60:
            pass
        else:
    ...
    def onPlayBackEnded(self):
        try:
            autoplay = control.setting('autoplay')
            if self.meta['episode'] and self.nextEP and autoplay == 'true':
                try:
                    self.nextEP = False
                    with open(control.episodesFile) as f:
                        data = json.load(f)
                    data = data[int(self.meta['episode'])]
                    print(self.meta['episode'])
                    sources.sources().play(data['title'], data['year'], data['imdb'], data['tvdb'], data['season'],
                                           data['episode'], data['tvshowtitle'], data['premiered'], data, 2)
...
class subtitles:
    def get(self, name, imdb, season, episode):
        try:
            ...
            langDict = {'Afrikaans': 'afr', 'Albanian': 'alb', 'Arabic': 'ara', 'Armenian': 'arm', 'Basque': 'baq',
                        'Bengali': 'ben', 'Bosnian': 'bos', 'Breton': 'bre', 'Bulgarian': 'bul', 'Burmese': 'bur',
                        'Catalan': 'cat', 'Chinese': 'chi', 'Croatian': 'hrv', 'Czech': 'cze', 'Danish': 'dan',
                        'Dutch': 'dut', 'English': 'eng', 'Esperanto': 'epo', 'Estonian': 'est', 'Finnish': 'fin',
                        'French': 'fre', 'Galician': 'glg', 'Georgian': 'geo', 'German': 'ger', 'Greek': 'ell',
                        'Hebrew': 'heb', 'Hindi': 'hin', 'Hungarian': 'hun', 'Icelandic': 'ice', 'Indonesian': 'ind',
                        'Italian': 'ita', 'Japanese': 'jpn', 'Kazakh': 'kaz', 'Khmer': 'khm', 'Korean': 'kor',
                        'Latvian': 'lav', 'Lithuanian': 'lit', 'Luxembourgish': 'ltz', 'Macedonian': 'mac',
                        'Malay': 'may', 'Malayalam': 'mal', 'Manipuri': 'mni', 'Mongolian': 'mon', 'Montenegrin': 'mne',
                        'Norwegian': 'nor', 'Occitan': 'oci', 'Persian': 'per', 'Polish': 'pol',
                        'Portuguese': 'por,pob', 'Portuguese(Brazil)': 'pob,por', 'Romanian': 'rum', 'Russian': 'rus',
                        'Serbian': 'scc', 'Sinhalese': 'sin', 'Slovak': 'slo', 'Slovenian': 'slv', 'Spanish': 'spa',
            ...
            if not (season == None or episode == None):
                result = server.SearchSubtitles(token, [
                    {'sublanguageid': sublanguageid, 'imdbid': imdbid, 'season': season, 'episode': episode}])['data']
                fmt = ['hdtv']
            else:
            ...
            for lang in langs:
                filter += [i for i in result if
                           i['SubLanguageID'] == lang and any(x in i['MovieReleaseName'].lower() for x in fmt)]
                filter += [i for i in result if
                           i['SubLanguageID'] == lang and any(x in i['MovieReleaseName'].lower() for x in quality)]
