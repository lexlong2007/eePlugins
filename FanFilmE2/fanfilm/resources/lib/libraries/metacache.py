...
def insert(meta):
    try:
        ...
        dbcur = dbcon.cursor()
        dbcur.execute(
            "CREATE TABLE IF NOT EXISTS meta (""imdb TEXT, ""tvdb TEXT, ""lang TEXT, ""user TEXT, ""item TEXT, ""time TEXT, ""UNIQUE(imdb, tvdb, lang, user)"");")
        t = int(time.time())
        for m in meta:
            try:
                ...
                try:
                    dbcur.execute(
                        ...
                        m['imdb'], m['lang'], m['user'], m['tvdb'], m['lang'], m['user']))
                except:
                    pass
