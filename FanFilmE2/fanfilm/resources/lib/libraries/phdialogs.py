...
def openDialog(image, audio):
    ...
    path = xbmc.translatePath(
        os.path.join('special://home/addons/plugin.video.phstreams/resources/skins/DefaultSkin', 'media'))
    ...
    if xbmc.getCondVisibility('system.platform.ios'):
        if not xbmc.getCondVisibility('system.platform.atv'):
            popup = dialog('pop1.xml', xbmcaddon.Addon().getAddonInfo('path'), 'DefaultSkin', close_time=20,
                           logo_path='%s/resources/skins/DefaultSkin/media/Logo/' % xbmcaddon.Addon().getAddonInfo(
                               'path'), )
    if xbmc.getCondVisibility('system.platform.android'):
        popup = dialog('pop1.xml', xbmcaddon.Addon().getAddonInfo('path'), 'DefaultSkin', close_time=20,
                       logo_path='%s/resources/skins/DefaultSkin/media/Logo/' % xbmcaddon.Addon().getAddonInfo('path'))
    else:
        popup = dialog('pop.xml', xbmcaddon.Addon().getAddonInfo('path'), 'DefaultSkin', close_time=20,
                       logo_path='%s/resources/skins/DefaultSkin/media/Logo/' % xbmcaddon.Addon().getAddonInfo('path'))
    popup.doModal()
    del popup
    ...
    try:
        ...
        if cookie:
            ...
            cookie_file = os.path.join(
                os.path.join(xbmc.translatePath(xbmcaddon.Addon().getAddonInfo('profile')), 'Cookies'),
                cookie + '.cookies')
...
class dialog(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        xbmc.Player().play(
            '%s/resources/skins/DefaultSkin/media/tempsound.mp3' % xbmcaddon.Addon().getAddonInfo('path'))  # Music
