...
class F4MDownloader():
    ...
    def preDownoload(self):
        ...
        try:
            ...
            self.bootstrap, self.boot_info, self.fragments_list, self.total_frags = self.readBootStrapInfo(bootstrapURL,
                                                                                                           bootstrapData)
            self.init_done = True
            return True
        except:
    ...
    def downloadInternal(self, url, dest_stream, segmentToStart=None, totalSegmentToSend=0):
        ...
        try:
            ...
            bootstrap, boot_info, fragments_list, total_frags = (
            self.bootstrap, self.boot_info, self.fragments_list, self.total_frags)
                ...
                if 1 == 1:
                    ...
                    while True:
                        ...
                        if box_type == b'mdat':
                            ...
                            # break
                self.status = 'play'
                if self.seqNumber == len(fragments_list) or (
                        totalSegmentToSend > 0 and frameSent == totalSegmentToSend):
                    if not self.live:
                        break
                    self.seqNumber = 0
                    ...
                    total_frags = None
                    try:
                        bootstrap, boot_info, fragments_list, total_frags = self.readBootStrapInfo(self.bootstrapURL,
                                                                                                   None,
                                                                                                   updateMode=True,
                                                                                                   lastSegment=seg_i,
                                                                                                   lastFragement=frag_i)
                    except:
                        traceback.print_exc()
                        pass
                    if total_frags == None:
