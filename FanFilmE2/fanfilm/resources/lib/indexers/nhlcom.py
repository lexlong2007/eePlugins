...
def nhlDirectory():
    ...
    for item in items:
        try:
            ...
            name = '%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s)[/COLOR]' % (
            item['a'], item['h'], est.strftime('%H:%M'), est.strftime('%Y-%m-%d'))
            url = str(item['id'])
    ...
    for item in items:
        try:
            try:
                ...
                est = re.findall('(\d{4})(\d{2})(\d{2}) (\d*:\d*)', item['est'])[0]

            try:
                date = int(est.strftime('%Y%m%d'))
            except:
                date = int(est[0] + est[1] + est[2])

            if not date <= datex: raise Exception()

            try:
                name = '%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s)[/COLOR]' % (
                item['awayTeamName'], item['homeTeamName'], (procTimezone(5, est)).strftime('%H:%M'),
                (procTimezone(5, est)).strftime('%Y-%m-%d'))
            except:
                name = '%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s-%s-%s)[/COLOR]' % (
                item['awayTeamName'], item['homeTeamName'], est[3], est[0], est[1], est[2])
...
def nhlArchives():
    ...
    for item in items:
        try:
            ...
            try:
                name = '%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s)[/COLOR]' % (
                item['a'], item['h'], (procTimezone(5, est)).strftime('%H:%M'),
                (procTimezone(5, est)).strftime('%Y-%m-%d'))
            except:
                name = '%s at %s  [COLOR gold](%s)[/COLOR]  [COLOR red](%s-%s-%s)[/COLOR]' % (
                item['a'], item['h'], est[3], est[0], est[1], est[2])
...
def nhlStreams(name, url):
    try:
        ...
        n1 = name + ' [COLOR gold]%s[/COLOR]';
        n2 = name + ' [COLOR red]%s[/COLOR]'

        selectHomeGame = 'x0xe%sx0xehome' % str(url)
        selectAwayGame = 'x0xe%sx0xeaway' % str(url)

        url = re.compile('(\d{4})(\d{2})(\d{4})').findall(url)[0]
        url = 'http://smb.cdnak.neulion.com/fs/nhl/mobile/feed_new/data/streams/%s/ipad/%s_%s.json' % (
        url[0], url[1], url[2])
    ...
    try:
        image = re.compile('"image" *: *"(.+?)"').findall(json.dumps(h))[-1]
    except:
        image = '0'

    try:
        l1.append({'name': n1 % 'Home LIVE', 'url': h['live']['bitrate0'] + selectHomeGame, 'image': image})
    except:
        pass

    try:
        l2.append({'name': n2 % 'Home Whole', 'url': h['vod-whole']['bitrate0'], 'image': image})
    except:
        pass

    try:
        l2.append({'name': n2 % 'Home Continuous', 'url': h['vod-continuous']['bitrate0'], 'image': image})
    except:
        pass

    try:
        l2.append({'name': n2 % 'Home Condensed', 'url': h['vod-condensed']['bitrate0'], 'image': image})
    except:
        pass

    try:
        image = re.compile('"image" *: *"(.+?)"').findall(json.dumps(a))[-1]
    except:
        image = '0'

    try:
        l1.append({'name': n1 % 'Away LIVE', 'url': a['live']['bitrate0'] + selectAwayGame, 'image': image})
    except:
        pass

    try:
        l2.append({'name': n2 % 'Away Whole', 'url': a['vod-whole']['bitrate0'], 'image': image})
    except:
        pass

    try:
        l2.append({'name': n2 % 'Away Continuous', 'url': a['vod-continuous']['bitrate0'], 'image': image})
    except:
        pass

    try:
        l2.append({'name': n2 % 'Away Condensed', 'url': a['vod-condensed']['bitrate0'], 'image': image})
    except:
        pass
...
def nhlResolve(url):
    try:
        try:
            url, selectGame, side = re.compile('(.+?)x0xe(.+?)x0xe(.+?)$').findall(url)[0]
        except:
...
def addDirectoryItem(name, url, action, image, fanart, isFolder=True):
    ...
    u = '%s?name=%s&url=%s&image=%s&fanart=%s&action=%s' % (
    sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(image), urllib.quote_plus(fanart),
    str(action))
