...
class indexer:
    ...
    def private(self):
        ...
        pb_list = [base64.b64decode('aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3LyVz') % i for i in pb_list if
                   not i == '' and len(i) == 8]
        ...
        if skip_m:
            ...
            if select == -1:
                ...
                result = client.request(dialog_list[select][0])
    ...
    def parental_controls(self):
        ...
        if os.path.isfile(PARENTAL_FILE):
            ...
            choice = dialog.yesno(AddonTitle, 'To use the ADULT section you must set a password.',
        ...
        try:
            ...
            if 'Tuner' in action:
                for i in self.list: i.update(
                    {'name': i['title'], 'poster': i['image'], 'action': 'plugin', 'folder': False})
                if 'Tuner2' in action: self.list = sorted(self.list, key=lambda x: random.random())
                self.addDirectory(self.list, queue=True)
            else:
                for i in self.list: i.update(
                    {'name': i['title'], 'poster': i['image'], 'nextaction': action, 'action': 'play', 'folder': False})
                self.addDirectory(self.list)
    ...
    def tvtuner(self, url):
        try:
            ...
            url, imdb, tvdb, tvshowtitle, year, thumbnail, fanart = re.findall('<url>(.+?)</url>', url)[0], \
                                                                    re.findall('<imdb>(.+?)</imdb>', url)[0], \
                                                                    re.findall('<tvdb>(.+?)</tvdb>', url)[0], \
                                                                    re.findall('<tvshowtitle>(.+?)</tvshowtitle>', url)[
                                                                        0], re.findall('<year>(.+?)</year>', url)[0], \
                                                                    re.findall('<thumbnail>(.+?)</thumbnail>', url)[0], \
                                                                    re.findall('<fanart>(.+?)</fanart>', url)[0]
            ...
            items = [(str(i.get('season')), str(i.get('number')), i.get('name').strip(), i.get('airdate')) for i in
                     items]
    ...
    def rotten_list(self, url):
        ...
        if u:
            ...
            r = [(i[0][0].content, i[1][0].content.replace('(', '').replace(')', ''), i[2][0].content,
                  i[3][0].attrs['src'], i[4][0].content) for i in r if i[0] and i[1] and i[2] and i[3] and i[4]]
        else:
            ...
            r = [(i[0][1].content, i[1][0].content.replace('(', '').replace(')', ''), i[2][0].content,
                  i[3][0].attrs['src'], re.sub('<.+?>', '', i[4][0].content)) for i in r if
                 i[0] and i[1] and i[2] and i[3] and i[4]]
        ...
        if r:
            for i in r:
                try:
                    ...
                    url = '<sublink><preset>search</preset>        <content>movie</content>        <imdb>%s</imdb>        <title>%s</title>        <year>%s</year>        </sublink>        <sublink><preset>searchsd</preset>        <content>movie</content>        <imdb>%s</imdb>        <title>%s</title>        <year>%s</year>        </sublink>        <thumbnail>%s</thumbnail>' % (
                    '0', original_name, i[1], '0', original_name, i[1], i[3])
                    self.list.append(
                        {'name': name, 'vip': '0', 'url': url, 'action': 'play', 'folder': False, 'poster': i[3],
                         'banner': '0', 'fanart': control.addonInfo('fanart'), 'content': 'movies', 'imdb': '0',
                         'tvdb': '0', 'tmdb': '0', 'title': original_name, 'originaltitle': original_name,
                         'tvshowtitle': '0', 'year': i[1], 'premiered': '0', 'season': '0', 'episode': '0',
                         'worker': '0', 'rating': i[2], 'plot': client.replaceHTMLCodes(i[4])})
                except:
                    log_utils.log('Scraping Error: Could not add %s to directory.' % str(original_name),
                                  log_utils.LOGERROR)
            return self.list
    ...
    def imdb_list(self, url):
        ...
        if url.startswith('http'):
            ...
            i_url = 'http://www.imdb.com/list/%s/' % url
        ...
        r = [(i[0][0].attrs['data-const'], i[1][1].content,
              ...
              i[5][0].content if i[5][0].content else '0.0',
              str(int(re.findall('\((\d+)\s*mins', i[3][0].content)[0]) * 60)) \
        ...
        if r:
            for i in r:
                try:
                    ...
                    url = '<sublink><preset>search</preset>        <content>movie</content>        <imdb>%s</imdb>        <title>%s</title>        <year>%s</year>        </sublink>        <sublink><preset>searchsd</preset>        <content>movie</content>        <imdb>%s</imdb>        <title>%s</title>        <year>%s</year>        </sublink>        <thumbnail>%s</thumbnail>' % (
                    i[0], original_name, i[2], i[0], original_name, i[2], i[4])
                    self.list.append(
                        {'name': name, 'vip': '0', 'url': url, 'action': 'play', 'folder': False, 'poster': i[4],
                         'banner': '0', 'fanart': control.addonInfo('fanart'), 'content': 'movies', 'imdb': i[0],
                         'tvdb': '0', 'tmdb': '0', 'title': original_name, 'originaltitle': original_name,
                         'tvshowtitle': '0', 'year': i[2], 'premiered': '0', 'season': '0', 'episode': '0',
                         'worker': '0', 'plot': i[3], 'rating': i[5], 'duration': i[6]})
                except:
                    log_utils.log('Scraping Error: Could not add %s to directory.' % str(original_name),
                                  log_utils.LOGERROR)
            return self.list
    ...
    def bennu_list(self, url, result=None, enc=False):
        ...
        try:
            ...
            if not any(x in result for x in checks):
                result = self.aes_dec(result)
            elif enc == True:
                result = self.aes_dec(result)
        ...
        for item in items:
            try:
                ...
                try:
                    use_worker = re.findall('<worker>(.+?)</worker>', item)[0]
                except:
                    use_worker = '0'
                ...
                try:
                    imdb_list = re.findall('<imdb_list>(.+?)</imdb_list>', item)[0]
                except:
                    imdb_list = False
                if imdb_list:
                    action = 'imdb_list';
                ...
                try:
                    trakt_list = re.findall('<trakt_list>(.+?)</trakt_list>', item)[0]
                except:
                    trakt_list = False
                if trakt_list:
                    action = 'trakt_list';
                ...
                try:
                    rotten_list = re.findall('<rotten_list>(.+?)</rotten_list>', item)[0]
                except:
                    rotten_list = False
                if rotten_list:
                    action = 'rotten_list';
                ...
                try:
                    yt_channel = re.findall('<yt_channel>(.+?)</yt_channel>', item)[0]
                except:
                    yt_channel = False
                if yt_channel:
                    action = 'youtubeChannel';
                    url = yt_channel;
                    folder = True
                ...
                self.list.append(
                    {'name': name, 'vip': vip, 'url': url, 'action': action, 'folder': folder, 'poster': image2,
                     ...
                     'premiered': premiered, 'season': season, 'episode': episode, 'worker': use_worker})
            except:
                pass
    ...
    def movie_info(self, i):
        try:
            ...
            if 'Error' in item and 'incorrect imdb' in item['Error'].lower():
                return self.meta.append({'imdb': imdb, 'tmdb': '0', 'tvdb': '0', 'lang': self.lang, 'user': self.user,
                                         'item': {'code': '0'}})
    ...
    def tv_info(self, i):
        try:
            ...
            if item[1] == '404':
                return self.meta.append({'imdb': '0', 'tmdb': '0', 'tvdb': tvdb, 'lang': self.lang, 'user': self.user,
                                         'item': {'code': '0'}})
            ...
            self.meta.append({'imdb': imdb, 'tmdb': '0', 'tvdb': tvdb, 'lang': self.lang, 'user': self.user,
    ...
    def addDirectory(self, items, queue=False, cache=True):
        ...
        for i in items:
            try:
                ...
                if build:
                    ...
                    try:
                        privurl = dict(urlparse.parse_qsl(urlparse.urlparse(url).query))['action']
                    except:
                        privurl = None
                    ...
                    if fanart == '0':
                        try:
                            fanart = i['fanart'] if not 'fanart.tv' in i['fanart'] else '0'
                        except:
                            fanart = '0'
                    ...
                    if poster == '0':
                        ...
                        poster = re.sub('([a-zA-Z]+)\d+_([a-zA-Z]+)\d+,\d+,\d+,\d+', '\g<1>512_\g<2>0,0,0,512', poster)
                    ...
                    if control.setting('metadata') == 'true':
                        try:
                            name = meta['title'] + ' (%s)' % meta['year'] if not meta['year'] == '0' else meta['title']
                        except:
                            pass
                    ...
                    if content in ['movies', 'tvshows']:
                        ...
                        cm.append((control.lang(30707).encode('utf-8'),
                                   'RunPlugin(%s?action=trailer&name=%s)' % (sysaddon, urllib.quote_plus(name))))

                    if content in ['movies', 'tvshows', 'seasons', 'episodes']:
                        cm.append((control.lang(30708).encode('utf-8'), 'XBMC.Action(Info)'))

                    if (folder == False and not '|regex=' in str(i.get('url'))) or (
                            folder == True and content in ['tvshows', 'seasons']):
                    ...
                    if devmode == True:
                        try:
                            cm.append(('Open in browser',
                                       'RunPlugin(%s?action=browser&url=%s)' % (sysaddon, urllib.quote_plus(i['url']))))
                        except:
                            pass
                    ...
                    try:
                        item.setArt(
                            {'poster': poster, 'tvshow.poster': poster, 'season.poster': poster, 'banner': banner,
        ...
        try:
            ...
            item.setArt(
                {'addonPoster': addonPoster, 'thumb': addonPoster, 'poster': addonPoster, 'tvshow.poster': addonPoster,
                 'season.poster': addonPoster, 'banner': addonPoster, 'tvshow.banner': addonPoster,
...
class resolver:
        ...
        try:
            ...
            for i in items:
                if i[0] != '':
                    ...
                    dialog_list += [('AUTOPLAY (Best available stream - can be false at times)',
                                     i[1].replace('<preset>search</preset>', '<preset>searchauto</preset>'))]
                    if not debrid.status() == False:
                        dialog_list += [('Premium (Real Debrid, Premiumize, etc)',
                                         i[1].replace('<preset>search</preset>', '<preset>searchrd</preset>'))]
    ...
    def f4m(self, url, name):
        try:
            ...
            try:
                ...
                streamtype = 'TSDOWNLOADER' if ext == 'ts' else 'HLS'
            ...
            return f4mProxyHelper().playF4mLink(url, name, proxy, proxy_use_chunks, maxbitrate, simpleDownloader,
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            try:
                title, year, imdb = re.findall('<title>(.+?)</title>', url)[0], re.findall('<year>(.+?)</year>', url)[
                    0], re.findall('<imdb>(.+?)</imdb>', url)[0]
            except:
                title, year, imdb = re.findall('<tvshowtitle>(.+?)</tvshowtitle>', url)[0], \
                                    re.findall('<year>(.+?)</year>', url)[0], re.findall('<imdb>(.+?)</imdb>', url)[0]
            ...
            if preset == 'search':
                quality = 'HD'
            elif preset == 'searchrd':
                quality = 'RD'
            elif preset == 'searchauto':
                quality = 'AUTO'
            else:
                quality = 'SD'
        ...
        try:
            ...
            from resources.lib.libraries import filmon
