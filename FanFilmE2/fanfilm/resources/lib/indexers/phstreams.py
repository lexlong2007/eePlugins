...
def getDirectory(name, url, audio, image, fanart, playable, content, close=True, local=False):
    ...
    special = re.compile(
        '<name>([^<]+)</name><link>([^<]+)</link><thumbnail>([^<]+)</thumbnail><date>([^<]+)</date>').findall(result)
    ...
    special = re.compile(
        '<name>([^<]+)</name><link>([^<]+)</link><thumbnail>([^<]+)</thumbnail><mode>([^<]+)</mode>').findall(result)
    ...
    for item in items:
        try:
            ...
            try:
                image = image2 = re.findall('<thumbnail>(.+?)</thumbnail>', item)[0]
            except:
                image = image2 = '0'
            ...
            if meta == True and content == 'movies':
                ...
                try:
                    ...
                    pass
                try:
                    data.update({'year': tvd['year'], 'imdb_id': tvd['imdb_id'], 'tvdb_id': tvd['tvdb_id'],
                                 'tvshowtitle': tvd['TVShowTitle'], 'genre': tvd['genre'], 'studio': tvd['studio'],
                                 'status': tvd['status'], 'duration': tvd['duration'], 'rating': tvd['rating'],
                                 'mpaa': tvd['mpaa'], 'plot': tvd['plot'], 'cast': tvd['cast']})
            ...
            if 'sublink' in url:
                addDirectoryItem(name, url, 'subDirectory', image, image2, fanart2, '0', content, data, tvshow=tvshow,
                                 totalItems=totalItems, isFolder=True)
            else:
                addDirectoryItem(name, url, 'resolveUrl', image, image2, fanart2, '0', content, data,
                                 totalItems=totalItems, isFolder=False)
        except:
            pass
...
def subDirectory(name, url, audio, image, fanart, playable, tvshow, content):
    ...
    try:
        ...
        data.update({'year': tvd['year'], 'imdb_id': tvd['imdb_id'], 'tvdb_id': tvd['tvdb_id'],
                     'tvshowtitle': tvd['TVShowTitle'], 'genre': tvd['genre'], 'studio': tvd['studio'],
                     'status': tvd['status'], 'duration': tvd['duration'], 'rating': tvd['rating'], 'mpaa': tvd['mpaa'],
                     'plot': tvd['plot'], 'cast': tvd['cast']})
...
def getSearch():
    ...
    try:
        ...
        for q in result:
            try:
                addDirectoryItem('%s...' % q, q, 'searchDirectory2', '0', '0', '0', '0', '0', {})
            except:
                pass
    except:
        pass
...
def searchDirectory(query=None):
    ...
    def search():
        return [x for y, x in enumerate((result + [query])) if x not in (result + [query])[:y]]
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    try:
                        image = re.findall('<thumbnail>(.+?)</thumbnail>', item)[0]
                    except:
                        image = '0'
                    ...
                    if 'sublink' in url:
                        addDirectoryItem(name, url, 'subDirectory', image, image, fanart2, '0', content, data,
                                         isFolder=True)
                    else:
                        addDirectoryItem(name, url, 'resolveUrl', image, image, fanart2, '0', content, data,
                                         isFolder=False)
                except:
                    pass
        except:
    ...
    try:
        ...
        if '</source>' in url:
            ...
            for i in ['_mv', '_tv', '_mv_tv']:
                try:
                    call = __import__('resources.lib.sources.%s%s' % (source, i), globals(), locals(), ['object'],
                                      -1).source()
                except:
                    pass
...
def addDirectoryItem(name, url, action, image, image2, fanart, audio, content, data, tvshow='0', totalItems=0,
                     isFolder=True):
    ...
    sysaddon, urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(audio), urllib.quote_plus(image),
    urllib.quote_plus(fanart), urllib.quote_plus(playable), str(tvshow), str(content), str(action))
    ...
    if content == 'movies' and not isFolder == True:
        ...
        try:
            ...
            pass
        cm.append((control.lang(30722).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (
        sysaddon, urllib.quote_plus(downloadFile), urllib.quote_plus(url), urllib.quote_plus(image))))

    elif content == 'episodes' and not isFolder == True:
        downloadFile = name
        try:
            downloadFile = '%s S%02dE%02d' % (data['tvshowtitle'], int(data['season']), int(data['episode']))
        except:
            pass
        cm.append((control.lang(30722).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (
        sysaddon, urllib.quote_plus(downloadFile), urllib.quote_plus(url), urllib.quote_plus(image))))
    ...
    try:
        item.setArt({'poster': image2, 'tvshow.poster': image2, 'season.poster': image2, 'banner': image,
                     'tvshow.banner': image, 'season.banner': image})
    except:
        pass
