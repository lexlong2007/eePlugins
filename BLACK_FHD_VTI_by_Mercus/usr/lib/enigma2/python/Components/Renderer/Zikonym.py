# Embedded file name: /usr/lib/enigma2/python/Components/Renderer/Zikonym.py
...
def onMountpointAdded(mountpoint):
    try:
        path = os.path.join(mountpoint, 'zzpicon') + '/'
        if os.path.isdir(path) and path not in searchPaths:
            for fn in os.listdir(path):
                if fn.endswith('.png'):
                    print '[Zikonym] adding path:', path
                    searchPaths.append(path)
                    break

    except Exception as ex:
        print '[Zikonym] Failed to investigate %s:' % mountpoint, ex
...
def onMountpointRemoved(mountpoint):
    ...
    try:
        ...
        print '[Zikonym] removed path:', path
    except:
        pass
