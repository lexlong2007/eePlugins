...
def executebuiltin(function):
    ...
    if not "Container.Update" in function:
        write_log("executebuiltin(%s) missing 'Container.Update' in function")
    else:    
        n1 = function.find("(", 0)
        n2 = function.find(")", n1)
        ...
        write_log("executebuiltin(%s) cmd = '%s'" % (function, cmd) )
        os.system(cmd)
...
def translatePath(url, asURL=False):
    ...
    write_log("translatePath(%s, asURL=%s)" % (url,str(asURL)))
    #print "In xbmc url A=", url
    xurl = url
    ...
    if "special://home/addons/" in url:
        url = url.replace("special://home/addons/", "/usr/lib/enigma2/python/Plugins/Extensions/KodiLite/plugins/")
